(ns puppetlabs.trapperkeeper.services.config.typesafe-test
  (:require [puppetlabs.config.typesafe :as ts]
            [puppetlabs.trapperkeeper.testutils.bootstrap :as bs]
...
(deftest configfile->map-test
  (testing "can parse .properties file with nested data structures"
    (let [cfg (ts/config-file->map (bs/file "config/file/config.properties"))]
      (is (= {:foo {:bar "barbar"
                    :baz "bazbaz"
                    :bam 42
  ...
  (testing "can parse .json file with nested data structures"
    (let [cfg (ts/config-file->map (bs/file "config/file/config.json"))]
      (is (= {:foo {:bar "barbar"
                    :baz "bazbaz"
                    :bam 42
                    :bap {:boozle "boozleboozle"
  ...
  (testing "can parse .conf file with nested data structures"
    (let [cfg (ts/config-file->map (bs/file "config/file/config.conf"))]
      (is (= {:foo {:bar "barbar"
                    :baz "bazbaz"
                    :bam 42
                    :bap {:boozle "boozleboozle"
